...
type Bot struct {
	...
	logFile        string
	workingDir     string // Add working directory field
	sudoPassword   string
}
...
func NewBot(config Config, logFile string) (*Bot, error) {
	...
	return &Bot{
		...
		logFile:        logFile,
		workingDir:     os.Getenv("."), // Initialize working directory
		sudoPassword:   "",                 // Initialize sudo password
	}, nil
}
...
func (b *Bot) executeCommand(command string, sudo bool) (string, error) {
	...
	if strings.HasPrefix(command, "cd ") {
		...
		b.workingDir, _ = os.Getwd() // Update working directory
		return fmt.Sprintf("Changed directory to: %s", b.workingDir), nil
	} else {
		cmd = exec.Command("/bin/sh", "-c", command)
		cmd.Dir = b.workingDir // Set working directory for the command
	}
...
// 	if len(args) != 2 {
// 		msg := tgbotapi.NewMessage(message.Chat.ID, "Usage: /sudopassword <password>")
// 		b.api.Send(msg)
// 		return
// 	}
// 	b.mu.Lock()
// 	b.sudoPassword = args[1]
// 	b.mu.Unlock()
//
// 	msg := tgbotapi.NewMessage(message.Chat.ID, "Sudo password set.")
// 	fmt.Println(b.sudoPassword)
// 	b.api.Send(msg)
// }
